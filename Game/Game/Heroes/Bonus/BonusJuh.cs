...
public class BonusJuh : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (HadAttack)
        {
            if (WaitTimeAttack < 0)
            {
                ...
                {
                    Player[i].GetComponent<Attack1Juh>().BasicAttack -= 2;
                    
                    GameObject magikprojekt = Player[i].GetComponent<Attack2Juh>().magicProjectile;
                    int MagicAttack = magikprojekt.GetComponent<ProjectileJuh>().MagicAttack;
                    MagicAttack -= 2;
                    Player[i].GetComponent<Attack2Juh>().magicProjectile.GetComponent<ProjectileJuh>().MagicAttack = MagicAttack;
                }
            ...
            else
            {
                if (HadSpeed)
                {
                    if (ColorUtility.TryParseHtmlString("#FF2FD4", out newcolor))
                        renderer.material.color = newcolor;
                }
                else
                {
                    if (ColorUtility.TryParseHtmlString("#FF0008", out newcolor))
                        renderer.material.color = newcolor;
                }
        ...
        if (HadSpeed)
        {
            ...
            else
            {
                ...
                {
                    if (ColorUtility.TryParseHtmlString("#0065FF", out newcolor))
                        renderer.material.color = newcolor;
                }
        ...
        if (HadHealth)
        {
            ...
            else
            {
                ...
                {
                    if (ColorUtility.TryParseHtmlString("#08FF00", out newcolor))
                        renderer.material.color = newcolor;
                }
                else
                { 
